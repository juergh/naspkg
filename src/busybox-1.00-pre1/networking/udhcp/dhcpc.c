...
struct client_config_t client_config = {
	...
	.interface = "egiga0",		//+Wilson03312006
	.pidfile = NULL,
	...
	.fqdn = NULL,
	.ifindex = 0,
	.def_ip = "192.168.0.32",   //+Wilson04202007
...
#endif
{
	...
	int max_fd;
	int sig;
	char buf[256];		//royc20040806
  int no_clientid = 0;
	FILE *fp; //jack20050601+
	...
	static const struct option arg_options[] = {
		...
		{"defaultip", required_argument,0, 'x'}, 	//royc20040806
		{0, 0, 0, 0}
	};
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "c:CV:fbH:h:F:i:np:qr:s:vx:", arg_options, &option_index);
		if (c == -1) break;
		...
		//royc 20040806
		case 'x':
			client_config.def_ip = optarg;
			printf("*****8%s\n", client_config.def_ip);
			break;
		//royc end
		default:
			show_usage();
			...
			switch (state) 
			{
				...
				else 
				{
					...
					if (client_config.background_if_no_lease) 
					{
						...
						//royc20040806
						sprintf(buf, "ifconfig egiga0 %s", client_config.def_ip);
						printf("------------>%s\n", buf);
						system(buf);
						system("route add -net 224.0.0.0 netmask 255.0.0.0 dev egiga0");
						system("write_ssl&");
						//amy.alpha 20090223
						//royc
						client_background();
		...
		else if (retval > 0 && listen_mode != LISTEN_NONE && FD_ISSET(fd, &rfds)) 
		{
			...
			case REBINDING:
				if (*message == DHCPACK) 
				{
					...
					//jack20060601+
					sprintf(buf,"%ld",lease / 2);
					if ( (fp = fopen("/tmp/LeaseTime","w+")) != NULL )
					{
						fputs(buf,fp);
						fclose(fp);
					}
