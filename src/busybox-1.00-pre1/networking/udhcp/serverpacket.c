...
int sendOffer(uint8_t *hostname, struct dhcpMessage *oldpacket)
{
	DBG_PRINT(printf("[serverpacket] sendOffer\n");)
	...
	if(!static_lease_ip)
	{
		DBG_PRINT(printf("[serverpacket] static_lease_ip is NULL\n");)
		   				...
		   				&&
							...
							/* and is not already taken/offered */
		   					((!(lease = find_lease_by_yiaddr(req_align)) 
		   						...
		   						lease_expired(lease))) ) 
		{
			DBG_PRINT(printf("[serverpacket] the client has a requested ip\n");)
			printf(" 	/* Or the client has a requested ip *\n");
			packet.yiaddr = req_align; 
			/* FIXME: oh my, is there a host using this IP? */
		} 
		/* otherwise, find a free IP */
		else 
		{
			DBG_PRINT(printf("[serverpacket] otherwise, find a free IP\n");)
		...
		DEBUG(LOG_INFO, "[serverpacket] sendOffer:hostname= %s\n", hostname);
		
		DBG_PRINT(printf("[serverpacket] add_lease\n");)	
		if (!add_lease(hostname, packet.chaddr, packet.yiaddr, server_config.offer_time)) //+Wilson03222006
		{	
...
int sendACK(uint8_t *hostname, struct dhcpMessage *oldpacket, uint32_t yiaddr)
{
	struct dhcpMessage packet;
	struct option_set *curr;
	uint8_t *lease_time;
	...
	DEBUG(LOG_INFO, "serverpacket(sendACK):::hostname= %s\n", hostname);
