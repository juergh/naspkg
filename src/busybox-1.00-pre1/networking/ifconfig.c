 ...
 *
 * $Id: ifconfig.c,v 1.1.1.1 2008/07/31 08:11:00 ffna Exp $
 *
 */
...
//royc add 20031202
int my_ifconfig_main(char *ifname, char *ifip)
{
	int argc = 8;
	char argv[9][16];
	char *cmd_end = "endcmd";
	...
	strcpy(argv[2], ifip);
	strcpy(argv[3], "netmask");
	strcpy(argv[4], "255.255.0.0");
	strcpy(argv[5], "broadcast");
	strcpy(argv[6], "169.254.255.255");
	strcpy(argv[7], "up");
	strcpy(argv[8], cmd_end);
...
#ifdef CONFIG_FEATURE_IFCONFIG_STATUS
	if ((argc > 0) && (((*argv)[1] == '-') && ((*argv)[2] == 'a') && !(*argv)[3]))
	{
		interface_opt_a = 1;
		--argc;
		//++argv;
	...
	/* get interface name */
	safe_strncpy(ifr.ifr_name, argv[ii], IFNAMSIZ);

	/* Process the remaining arguments. */
	//while (*++argv != (char *) NULL)
	//while (argv[++ii] != (char *) NULL)
	while( strcmp(argv[++ii], cmd_end) != 0)
	{
		p = argv[ii];
		mask = N_MASK;
		if (*p == '-') {	/* If the arg starts with '-'... */
			  ...
			  HOSTNAME:
				...
				if (mask & A_CAST_HOST_COPY) {
					...
					if (mask & A_CAST_RESOLVE) {
						...
						safe_strncpy(host, argv[ii], (sizeof host));
#ifdef CONFIG_FEATURE_IPV6
						if ((prefix = strchr(host, '/'))) {
							prefix_len = atol(prefix + 1);
							if ((prefix_len < 0) || (prefix_len > 128)) {
						...
						/* This is the "hw" arg case. */
						//if (strcmp("ether", *argv) || (*++argv == NULL))
						//if (strcmp("ether", argv[ii]) || (argv[++ii] == NULL))
						if (strcmp("ether", argv[ii]) || (strcmp(argv[++ii], cmd_end) == 0))
						{
							bb_show_usage();
						}
						safe_strncpy(host, argv[ii], (sizeof host));
					...
					unsigned int i = strtoul(argv[ii], NULL, 0);
