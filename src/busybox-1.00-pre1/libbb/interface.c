 ...
 *
 * Version:     $Id: interface.c,v 1.1.1.1 2008/07/31 08:11:00 ffna Exp $
 *
...
char* get_hw_info(char *ifname)
{
	...
	hw = get_hwntype(hf);
	//printf(_("HWaddr %s  "), hw->print(ife->hwaddr));
	
	close(skfd);
	return (hw->print(ife->hwaddr));
}

void get_ip_info(char *ifname, char *ip_str)
{
	int hf;
	int res;
	struct interface *ife;
	...
	strcpy(ip_str, ap->sprint(&ife->addr, 1) );
}
